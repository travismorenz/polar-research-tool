...
if __name__ == "__main__":
  ...
  parser.add_argument('--startidx', type=int, help='The index of title in list to start from. For resuming from errors.')
  parser.add_argument('--results-per-iteration', type=int, default=5, help='passed to arxiv API')
  parser.add_argument('--wait-time', type=float, default=2.0, help='lets be gentle to arxiv API (in number of seconds)')
  ...
  titles = ['ti:"' + '+'.join(t.strip().split()) + '"' for t in titles]#+ '+AND+cat:cs.CV+OR+cat:cs.AI+OR+cat:cs.LG+OR+cat:cs.CL+OR+cat:cs.NE+OR+cat:stat.ML'     for t in titles]
