# Adversarial Sanity Preserver

This repo contains a clone of arxiv sanity, with a database restricted to articles about adversarial machine learning
for the most part. The database was collected using a restriced set of queries to the arxiv API for key phrases:
...
In addition queries were made for title matches for all content on our confluence lit-review pages.
The database contains 539 articles which we can serve with search and similarity capabilities using the 
arxiv sanity set up. 

There is also code for topic modeling and topic visualization in topic.ipynb.

## arxiv sanity preserver read me content
